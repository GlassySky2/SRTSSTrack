...
# MAX_MISSES = 60  #
CONFIRMED_LIMIT = 60  # SUTrack 锁定目标最大允许丢 60 帧
...
class Track:
    ...
    def update_by_sutrack(self, frame_rgb):
        ...
        # --- 核心改动：无条件更新 self.box ---
        # 即使 score 只有 0.1，也强制把框移过去
        self.box = new_box

        cx, cy = self.box[0] + self.box[2] / 2, self.box[1] + self.box[3] / 2

        # 更新缓存坐标，用于下一帧逻辑（虽然此时你可能不太在乎它的预测了）
        self.cached_predict = (cx, cy)
...
def run_system1(video_path, save_path,sutrack_cfg="sutrack_b224"):
    ...
    while cap.isOpened():
        ...
        for t in tracker.tracks:
            ...
            if t.st_engine is not None:  # 已确认航迹 (Confirmed)
                active_count += 1

                # --- 核心改进：区分锁定与盲跑 ---
                if t.misses == 0:
                    color = (0, 255, 0)  # 锁定：明亮绿色
                    label = f"ID:{t.track_id}"
                    thickness = 2
                else:
                    # 盲跑：随着丢失帧数增加，颜色变淡或变灰，提示正在找回
                    color = (128, 128, 128)  # 盲跑：灰色
                    label = f"ID:{t.track_id} LOST:{t.misses}"
                    thickness = 1

                cv2.rectangle(frame, (x, y), (x + w, y + h), color, thickness)
                cv2.putText(frame, label, (x, y - 10), 1, 1, color, thickness)
                ...
                # 建议给 Pending 也显示一下命中计数，方便调试起批门槛
                # cv2.putText(frame, f"P", (x, y - 5), 1, 0.8, (0, 255, 255), 1)
...
def batch_run():
    ...
    video_dir = Path("test_videos/2026.1.15_2")
    result_dir = Path("results/2026.1.15_2")
...
if __name__ == "__main__":
    ...
    SAVE_PATH = "./results/2026.1.08/DJI_20260110144928_0011_W.MP4"
    run_system1(VIDEO_PATH, SAVE_PATH,sutrack_cfg="sutrack_b224")
    # batch_run()