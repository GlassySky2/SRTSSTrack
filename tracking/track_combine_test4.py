...
# --- 航迹管理系统 ---
class Track:
    def __init__(self, box, manager=None, frame_rgb=None,score=0.0):
        self.track_id = -1
        ...
        # --- 【核心新增：能量池逻辑】 ---
        self.energy = score  # 初始能量池
        self.energy_momentum = 0.75  # 动量因子，保留上一帧能量的比例
    ...
    def update_by_cv(self, box,score,manager, frame_rgb):
        """阶段 A：由传统检测坐标更新"""
        """阶段 A：由传统检测坐标更新，加入能量累加"""
        cx, cy = box[0] + box[2] / 2, box[1] + box[3] / 2
        ...
        # --- 【核心新增：能量累加计算】 ---
        # 能量 = 当前帧分数 + (历史能量 * 动量因子)
        self.energy = score + (self.energy * self.energy_momentum)
        ...
        # --- 【核心修改：转正判定条件】 ---
        # 原条件：sum(self.hit_window) >= HIT_THRESHOLD
        # 新条件：能量池满额 (例如 3.5 分)
        energy_threshold = getattr(manager, 'energy_threshold', 3.5)
        if not self.is_confirmed and self.energy >= energy_threshold: #sum(self.hit_window) >= HIT_THRESHOLD:
    ...
    def mark_missed(self,step=1):
        self.misses += step
        self.energy *= 0.5 # 丢失一帧，能量池削减一半，防止随机噪声通过累积混入
...
class TrackManager:
    def __init__(self,sutrack_params=None,max_confirmed = 10):
        ...
        # --- 在这里定义能量阈值 ---
        self.energy_threshold = 5.0  # 5.0 表示需要积累较多能量才能起批
    ...
    def _worker_cv_discovery(self, pending_tracks, confirmed_tracks,detections, frame_rgb):
        ...
        for p_idx, d_idx in zip(row_ind, col_ind):
            ...
            if cost_matrix[p_idx, d_idx] < dynamic_radius:
                ...
                pending_tracks[p_idx].update_by_cv(
                    det['box'],
                    det.get('score', 0.0),  # 传递分数
                    self,
                    frame_rgb
                )
                matched_p.add(p_idx)
        ...
        for j in range(num_d):
            if j not in matched_d:
                ...
                det = clean_detections[j]
                score = det.get('score', 0)
                # 起批门槛调低一点（比如 0.8），允许它进入能量池观察
                if score < 0.8: continue
                d_cx, d_cy = d_centers[j]
                is_redundant = any(np.hypot(d_cx - (p.box[0] + p.box[2] / 2),
                                            d_cy - (p.box[1] + p.box[3] / 2)) < 25 for p in pending_tracks)
                if not is_redundant:
                    # 【修改点】：初始化时传入首帧分数
                    self.tracks.append(Track(det['box'], score=score))
...
def run_system(video_path, save_path, sutrack_cfg="sutrack_b224"):
    ...
    # 【新增】设置能量起批阈值：针对极小目标，建议 3.5-4.5
    tracker.energy_threshold = 4.0
    ...
    cfg = {
        'CLUSTER_DIST': 30,
        'ALPHA': 0.3,
        'W_MOTION': 1.8,  # 略微提高运动权重，捕捉右偏趋势
        'W_AREA': 1.0,
        'W_SPATIAL': 1.5,
        'MOTION_FLOOR': 0.6,  # 【关键】允许更小的位移进入能量池
        'BUFFER_PIXELS': 0,
    }
    ...
    while cap.isOpened():
        ...
        if prev_gray is not None and prev_gray.shape == curr_gray.shape:
            ...
            if pts_ref is not None:
                ...
                good_prev, good_curr = pts_ref[st == 1], pts_curr[st == 1]
                if len(good_prev) >= 10:
            ...
            if M is not None:
                m_status = "LOCKED"
                y_cutoff = min(fh, int(np.max(sky_line) + 2))
                ...
                if len(clustered_cands) > 0:
                    # 向量化光流验证
                    pts_list = [[c['box'][0] + c['box'][2] / 2, c['box'][1] + c['box'][3] / 2] for c in clustered_cands]
                    ...
                    for i, cand in enumerate(clustered_cands):
                        if st_obj[i] == 1:
                            ...
                            if track_preds:
                                ...
                                if np.min(dists) < 40:
                                    is_near_track = True
                                    spatial_bonus = cfg['W_SPATIAL'] * max(0, (40 - np.min(dists)) / 8.0)

                            # 【核心评分累加准入】
                            if (net_motion > cfg['MOTION_FLOOR'] or is_near_track) and (net_motion < 40.0):
                                eff_motion = max(net_motion, 0.8 if is_near_track else 0.5)
                                frame_score = (cfg['W_MOTION'] * eff_motion) + (
                                            cfg['W_AREA'] * np.log1p(cand['area'] + 1)) + spatial_bonus

                                # 准入门槛降低，交给能量池做最终长线筛选
                                if frame_score > 0.8:
                                    cand['score'] = frame_score
                                    validated_with_score.append(cand)

        # 3. 聚类并送入 TrackManager 更新能量池
        ...
        for t in tracker.tracks:
            ...
            # --- 分支 A: 已激活 SUTrack 的确认目标 ---
            if t.st_engine is not None:
                active_count += 1
                color = (0, 255, 0) if t.is_drone_confirmed else (147, 20, 255)
                status_text = f"UAV ID:{t.track_id} {t.yolo_score:.2f}" if t.is_drone_confirmed else f"CHECK ID:{t.track_id} Neg:{t.negative_count}"
                if t.misses > 0:
                    color, status_text = (128, 128, 128), status_text + f" LOST:{t.misses}"
            ...
            # --- 分支 B: 正在攒分考察的 Pending 目标 ---
            else:
                # 【新增】显示当前累积能量
                curr_e = getattr(t, 'energy', 0.0)
                color = (0, 255, 255)
                cv2.rectangle(frame, (x, y), (x + w, y + h), color, 1)
                # 实时观察能量是否在往 tracker.energy_threshold 涨
                cv2.putText(frame, f"P E:{curr_e:.1f}", (x, y - 5), cv2.FONT_HERSHEY_SIMPLEX, 0.4, color, 1)
        ...
        if frame_count % 50 == 0:
            print(f"进度: {frame_count}/{total_frames} 帧 | 活动目标: {active_count}")
    ...
    print(f"\n处理完成：耗时 {duration:.2f}s | 平均速度 {frame_count / duration:.1f} FPS")
...
if __name__ == "__main__":
    # --- 全局配置 ---
    VIDEO_PATH = './test_videos/clip_videos/DJI_20260115175909_0002_W.MP4'
    SAVE_PATH = "./results/clip_videos/test.MP4"
    run_system(VIDEO_PATH, SAVE_PATH,sutrack_cfg="sutrack_b384")
    # batch_run()