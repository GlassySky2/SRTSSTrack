...
from lib.test.evaluation.uav_testdataset import UAVTestDataset
DatasetInfo = namedtuple('DatasetInfo', ['module', 'class_name', 'kwargs'])

pt = "lib.test.evaluation.%sdataset"  # Useful abbreviations to reduce the clutter
#预测模式	--dataset uav	UAVDataset	只读取第一帧框，生成轨迹 .txt，不计算 IoU（因为 Dataset 里没给真值）。
#测试模式	--dataset uav_test	UAVTestDataset	读取全序列真值，运行完后直接打印 Success/Precision 分数。
dataset_dict = dict(
...
# 2. 新增的测试评估用数据集 (指向 uav_testdataset.py)
    uav_test=DatasetInfo(module=pt % "uav_test", class_name="UAVTestDataset", kwargs=dict()),
