...
# 在其他 dataset 导入语句附近添加
from lib.train.dataset.uav_custom import UavCustom #新增
...
def names2datasets(name_list: list, settings, image_loader):
    ...
    for name in name_list:
        assert name in ["LASOT", "GOT10K_vottrain", "GOT10K_votval", "GOT10K_train_full",
                        ...
                        "REFCOCOG", "TNL2K_train", "OTB99_train","VASTTRACK","UAV_CUSTOM"]
        # 新增
        if name == "UAV_CUSTOM":
            datasets.append(UavCustom(settings.env.uav_dir,
                                      split='train',
                                      image_loader=image_loader,
                                      # 必须添加下面这一行，将 settings 里的开关传给数据集类
